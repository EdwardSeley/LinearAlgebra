        ...
        for x in range(len(data)):
            if type(data[x]) != int and type(data[x]) != float:
                raise ValueError("Components must be integers or decimals. Item "
                                 + str(data[x]) + "is type " + str(type(data[x])))
...
class Matrix:
    ...
    def __init__(self, data):
        ...
        for x in range(0, len(data)):
            if type(data[x]) != list:
                raise ValueError("Parameter must be a list of a integer or decimal rows. "
                                 "the item number " + str(x) + " in the list is not a list")
        ...
        self.entries = data
        self.columnSize = len(data)
        self.rowSize = len(data[0])
    ...
    def scalar_mult(self, scalar):
        ...
        if type(scalar) != int and type(scalar) != float:
            raise ValueError("Scalar must be an integer or decimal")
        scaled_matrix = [[0 for a in range(0, self.rowSize)] for b in range(0, self.columnSize)]
        for x in range(0, self.columnSize):
            for y in range(0, self.rowSize):
